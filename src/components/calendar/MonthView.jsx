// MonthView.jsx -- 6-week month grid. Each cell shows the date, an optional
// holiday banner, and up to 3 event chips ("+N more" beyond that). Reuses
// the same `dayBlocks` shape WeekView/Calendar.jsx already produce (one
// entry per visible day), just chunked into rows of 7.
...
export default function MonthView({ dayBlocks, monthAnchor, todayStr, holidaysByDate, t, onSelectDay, onOpenEvent, onAddEvent }) {
  const weeks = [];
  for (let i = 0; i < dayBlocks.length; i += 7) weeks.push(dayBlocks.slice(i, i + 7));
  const currentMonth = monthAnchor.getMonth();
  const weekdayLabels = dayBlocks.slice(0, 7).map((d) => d.date.toLocaleDateString("en-US", { weekday: "short" }).toUpperCase());
