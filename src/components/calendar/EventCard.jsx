...
export default function EventCard({ block, t, onOpen, onEdit, onDuplicate, onDelete }) {
  ...
  return (
    <div
      ...
      className={cn(
        "group absolute left-1 right-1 z-10 flex cursor-pointer flex-col overflow-hidden rounded-lg border border-l-4 bg-card px-2 py-1 text-left shadow-sm transition-shadow hover:z-20 hover:shadow-md",
    ...
    >
      <div className="flex items-start justify-between gap-1">
        ...
        <div className="hidden shrink-0 items-center gap-0.5 group-hover:flex">
          {editable && (
            <>
              <button type="button" aria-label={t("calendarEdit")} className="rounded p-0.5 text-muted hover:bg-card hover:text-card-foreground" onClick={(e) => { e.stopPropagation(); onEdit(block); }}>
                <Pencil className="h-3 w-3" />
              </button>
              <button type="button" aria-label={t("calendarDuplicate")} className="rounded p-0.5 text-muted hover:bg-card hover:text-card-foreground" onClick={(e) => { e.stopPropagation(); onDuplicate(block); }}>
                <Copy className="h-3 w-3" />
              </button>
              <button type="button" aria-label={t("calendarDelete")} className="rounded p-0.5 text-muted hover:bg-card hover:text-accent" onClick={(e) => { e.stopPropagation(); onDelete(block); }}>
