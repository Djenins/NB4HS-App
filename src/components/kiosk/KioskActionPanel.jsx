...
// panel (thank-you note + community illustration) with the Submit Check-In
// button anchored at the bottom. The post-submit confirmation is a separate
// route (CheckInSuccess.jsx) and is left exactly as it was, so this panel is
...
export default function KioskActionPanel({ title, description, submitLabel, busy }) {
  return (
    <aside className="flex min-h-0 flex-col overflow-y-auto rounded-[18px] lg:h-full border border-border bg-[#F5F8FE] p-5 text-center dark:bg-background xl:p-6">
      ...
      <SubmitCheckInButton label={submitLabel} busy={busy} className="mt-6" />
    </aside>
  );
}
